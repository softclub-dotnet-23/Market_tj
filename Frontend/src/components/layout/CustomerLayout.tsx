...
import { Bell, ChevronDown, ChevronsLeft, ChevronsRight, LayoutDashboard, Leaf, LogOut, Search, ShoppingCart, User as UserIcon } from "lucide-react";
...
export function CustomerLayout() {
  ...
  const currentItem = NAV_ITEMS.find((item) =>
    item.path === "/customer" ? location.pathname === "/customer" : location.pathname.startsWith(item.path),
  );
        ...
        <header className="flex h-18 shrink-0 items-center justify-between gap-4 border-b border-stone-100 bg-white px-6 dark:border-stone-800 dark:bg-stone-900">
          ...
          <div className="flex items-center gap-3">
            ...
            <div ref={menuRef} className="relative">
              ...
              >
                <Avatar name={user?.fullName ?? t("customerName")} size={36} />
                <ChevronDown
                  size={14}
