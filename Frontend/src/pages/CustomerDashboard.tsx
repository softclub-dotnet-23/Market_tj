...
import { ArrowRight, Clock, MapPin, PackageCheck, ShoppingBag, Wallet } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Hero — аватар, имя и ключевые цифры одним взглядом, без захода в
          ...
          уровней, которых у Market.tj просто нет). Декоративные размытые
          круги + стеклянные плашки статистики вместо плоской заливки. */}
      <div className="relative overflow-hidden rounded-3xl bg-linear-to-br from-grove-600 via-grove-700 to-grove-900 p-6 text-white shadow-(--shadow-lifted) sm:p-8">
        <div className="pointer-events-none absolute -top-24 -right-16 h-64 w-64 rounded-full bg-white/10 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-28 -left-12 h-72 w-72 rounded-full bg-harvest-400/20 blur-3xl" />
        ...
        <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1.5 flex items-center gap-1.5 truncate text-sm text-grove-100">
          ...
          <div className="grid grid-cols-3 gap-2.5 sm:flex sm:gap-3">
            <div className="flex items-center gap-2.5 rounded-2xl bg-white/10 px-3.5 py-3 backdrop-blur-sm sm:px-4">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/15">
                <ShoppingBag size={16} />
              </span>
              <div className="min-w-0">
                <p className="font-display text-xl leading-tight sm:text-2xl">{formatNumber(orders.length)}</p>
                <p className="truncate text-[11px] text-grove-100">{t("dashboard.stats.totalOrders")}</p>
              </div>
            </div>
            <div className="flex items-center gap-2.5 rounded-2xl bg-white/10 px-3.5 py-3 backdrop-blur-sm sm:px-4">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/15">
              ...
              <div className="min-w-0">
                <p className="font-display text-xl leading-tight sm:text-2xl">{formatNumber(activeCount)}</p>
                <p className="truncate text-[11px] text-grove-100">{t("dashboard.stats.activeOrders")}</p>
              </div>
            </div>
            <div className="flex items-center gap-2.5 rounded-2xl bg-white/10 px-3.5 py-3 backdrop-blur-sm sm:px-4">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/15">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-[11px] text-grove-100">{t("dashboard.stats.totalSpent")}</p>
              </div>
            </div>
          </div>
        </div>
