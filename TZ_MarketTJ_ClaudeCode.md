...
- **AI-ассистент (изначально Anthropic Claude API, с 2026-08-01 — Google
  Gemini API)** — осознанное отклонение от раздела 3 («В MVP не входят:
  искусственный интеллект»), подтверждено пользователем явно. Endpoint:
  `POST /api/ai-assistant/ask`, без `[Authorize]` — доступен и гостям
  (явный запрос пользователя, 2026-08-01). Модель — `gemini-2.0-flash`.
  Ключ — `Gemini:ApiKey`, пустая строка в `appsettings.json`, реальное
  значение — только в User Secrets. Причина смены провайдера: на тестовый
  период нужен бесплатный тариф без привязки карты (Anthropic — платный по
  токенам); после запуска с реальными клиентами решение можно пересмотреть.
  Изначально использовалась модель `claude-sonnet-5` (в присланном промпте
  было выдуманное имя `claude-sonnet-4-6` — исправлено).
  - Изначально — только покупатель (поиск по каталогу через `search_products`,
    возвращает `{intent: product|category|cart|orders|none, productId,
    categoryId, message}` для навигации на фронтенде).
  - С 2026-08-01 — роль-осознанный (по явному запросу пользователя: ассистент
    должен помогать не только клиенту, но и фермеру, и админу, включая
    выполнение действий, не только навигацию/справку). Роль/userId читаются
    из JWT через `ICurrentUserService` (`/ask` остаётся без `[Authorize]`,
    но ASP.NET заполняет `HttpContext.User`, если токен передан, даже без
    этого атрибута на маршруте) — гость видит только инструмент покупателя.
    Фермер получает `get_dashboard`/`get_my_listings`/`propose_update_listing`;
    админ — `get_dashboard`/`get_pending_verifications`/`get_pending_reports`/
    `propose_resolve_report`. Паттерн безопасности: ассистент только
    ПРЕДЛАГАЕТ действие (`intent: action_pending`, поле `Action`) — реальная
    мутация происходит только через отдельный `POST /api/ai-assistant/execute-action`
    (**с `[Authorize]`**, в отличие от `/ask`) после подтверждения
    пользователем на фронтенде, с повторной проверкой прав на сервере (не
    доверяет тому, что решил AI). Самые чувствительные admin-действия —
    одобрение/отклонение верификации фермера, блокировка пользователя —
    в этот проход намеренно не включены, им нужен отдельный UX, не
    одношаговый чат-confirm.
