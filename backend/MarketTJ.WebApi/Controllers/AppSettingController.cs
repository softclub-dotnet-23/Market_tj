...
public class AppSettingController(IAppSettingService service) : ApiControllerBase
{
    [HttpGet]
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateAppSettingDto dto)
        => HandleResult(await service.CreateAsync(dto));

    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateAppSettingDto dto)
