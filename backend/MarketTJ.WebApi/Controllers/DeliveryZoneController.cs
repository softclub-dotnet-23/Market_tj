...
public class DeliveryZoneController(IDeliveryZoneService service) : ApiControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateDeliveryZoneDto dto)
        => HandleResult(await service.CreateAsync(dto));

    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateDeliveryZoneDto dto)
